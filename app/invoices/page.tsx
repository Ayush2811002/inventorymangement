      ...
      if (data && typeof data === "object") {
        const invoicesData: Invoice[] = Object.entries(data).map(
          ([id, invoiceData]) => {
            ...
            const grandTotal =
              (invoice as unknown as { taxes?: Taxes })?.taxes?.grandTotal ?? 0;
    ...
    if (sortConfig) {
      result.sort((a, b) => {
        if (sortConfig.key === "amount") {
          const aAmount = a.products
            ? a.products.reduce((total, p) => total + p.qty * p.price, 0)
            : 0;
          const bAmount = b.products
            ? b.products.reduce((total, p) => total + p.qty * p.price, 0)
            : 0;
  ...
  const handleBulkDelete = async () => {
    ...
    if (confirm.isConfirmed) {
      try {
        ...
        Swal.fire(
          "Deleted!",
          `${selectedInvoices.length} invoice(s) have been removed.`,
          "success"
        );
      } catch (error) {
  ...
  const handleBulkStatusUpdate = async (status: string) => {
    ...
    if (confirm.isConfirmed) {
      try {
        ...
        const updateData =
          status === "paid"
            ? { paymentStatus: status, paidOn: today }
            : { paymentStatus: status, paidOn: "N/A" };

        // Create an array of promises for each update operation
        const updatePromises = selectedInvoices.map((id) =>
          update(ref(db, `invoices/${id}`), updateData)
        );
        ...
        Swal.fire(
          "Updated!",
          `${selectedInvoices.length} invoice(s) have been updated.`,
          "success"
        );
      } catch (error) {
                        ...
                        <TableHeader>
                          <TableRow className="border-gray-800 hover:bg-gray-900">
                            ...
                            >
                              <div className="flex items-center">
                                Client Name
                                {sortConfig && sortConfig.key === "billTo" && (
                                  <ArrowUpDown className="ml-2 h-4 w-4" />
                                )}
                              </div>
                            </TableHead>
                            ...
                            >
                              <div className="flex items-center">
                                Paid On
                                {sortConfig && sortConfig.key === "paidOn" && (
                                  <ArrowUpDown className="ml-2 h-4 w-4" />
                                )}
                              </div>
                            </TableHead>
                        ...
                        <TableBody>
                          {paginatedInvoices.map((invoice) => (
                            ...
                            >
                              ...
                              <TableCell className="text-gray-300">
                                ₹{Math.round(calculateAmount(invoice))}
                              </TableCell>
              ...
              <div className="space-y-2">
                ...
                {selectedInvoice.paymentStatus !== "paid" && (
                  <p className="text-xs text-gray-500">
                    Set status to &quot;Paid&quot; to enable date selection
                  </p>
                )}
              </div>
            </div>
